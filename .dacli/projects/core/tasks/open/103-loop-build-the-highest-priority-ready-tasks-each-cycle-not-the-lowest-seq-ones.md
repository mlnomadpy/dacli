...
## Acceptance
- [ ] readyTasks (or runCycle before slicing to width) ranks the ready frontier by MoSCoW priority rank first, then critical-path slack when a CPM schedule is available, then Seq as the final tiebreak — mirroring cmdNext's selection so the two readouts agree on what to work on first
- [ ] a regression test in internal/features/orchestration constructs a ready set where a low-seq 'could' and a high-seq 'must' are both ready and width=1, and asserts the 'must' is the one placed in the built batch
- [ ] the loop never hands the standing 'Continuous improvement' anchor task to a builder (existing behavior preserved)
