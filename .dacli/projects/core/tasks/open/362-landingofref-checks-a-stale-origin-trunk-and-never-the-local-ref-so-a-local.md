...
## Acceptance
- [ ] LandingOfRef (internal/store/landing.go:79-97) no longer returns LandingUnlanded for a commit that is an ancestor of the LOCAL trunk ref (refs/heads/<trunk>) merely because origin/<trunk> exists but is behind: when the resolved trunk refs disagree, a commit reachable from EITHER the local or the remote trunk ref is reported LandingLanded, not LandingUnlanded
...
- [ ] Red-green demonstrated in the commit message or a finding: the new test fails on current HEAD and passes after the fix
- [ ] The genuinely-unlanded case (a branch merged nowhere) still reports LandingUnlanded, and the --pr path (origin/<trunk> carries the merge via git pull --ff-only) still reports LandingLanded -- no regression to the dacli 329 / 342 behavior
## Log
