...
## Acceptance
- [ ] every clikit.Refusedf and Usagef is checked against the rule that a refusal must name the action that would succeed
- [ ] findings name the file:line and quote the current message and the missing instruction
- [ ] any refusal using the wrong exit class (a policy answer returned as exit 1, or a real failure returned as exit 3) is called out separately
## Log
