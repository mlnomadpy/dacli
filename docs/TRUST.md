...
so they carry nothing to document under "what gates it." The 61 commands below
...
| Command | What it changes | What gates it | How to undo it |
|---|---|---|---|
| `commit` | `git add -A` + `git commit` in `ctx.Cwd` (the agent's own worktree, if isolated) with `Dacli-Agent`/`Dacli-Role`/`Dacli-Task` trailers carrying the agent's *id*, never its token (§ 4); appends an `EventCommit` | Refuses to commit directly to the default branch; refuses when nothing is staged; **claim-scope enforcement** — refuses to stage files outside the agent's recorded `--claim` unless `--force`, so a spawned child cannot silently commit outside the tree it declared it would touch | `git reset --soft HEAD~1` / `git revert` in the same worktree/branch — ordinary git semantics; the recorded `EventCommit` itself is not auto-removed |
...
| `cleanup` | Removes only explicitly planned managed worktrees and their merged local branches; writes a content-addressed cleanup audit | Dispatcher rw baseline for apply; `--dry-run` is read-only. Apply recomputes the versioned plan and refuses a changed id. Eligibility requires canonical merge classification plus clean, pushed, terminal-task, non-protected, uniquely merged-PR evidence; unknown external state preserves the item | Each audit records the exact commit and commands to recreate the branch and worktree (`git branch recovered/... <sha>`; `git worktree add <path> <sha>`) |
...
| Command | What it changes | What gates it | How to undo it |
|---|---|---|---|
| `github link` | Writes `github_repo` (and, if the repo is public, `github_public_confirmed` scoped to that exact `owner/repo`) into the project's frontmatter — local only, no remote write | Live-visibility probe (`gh repo view`): a public repo with no matching recorded consent is refused, pointing at `--allow-public` | Edit/remove the `github_repo`/`github_public_confirmed` frontmatter lines, or re-link to a different repo (which requires fresh consent, since it's scoped to the exact repo) |
| `github push` | The most extensive remote-write command: creates/closes issues, creates labels and milestones, posts finding comments, files decision/finding issues (filed **closed** — a decision is a record, not open work), and writes the resulting `github:` id mapping back onto local task/note files | `disclosureGate` re-checks *live* visibility on every call, independent of what `github link` recorded at link time; an idempotency preflight refuses rather than risk mass-duplicating when the remote issue list can't be fully read; `--dry-run` runs the identical read/decision path and previews every create before any write | No revert command — close/relabel/delete issues, comments, milestones, and labels by hand on GitHub; clearing the local `github:` block un-maps it locally (a later push would then re-adopt-by-marker or create anew) |
