...
| `project show` | With `--landing-mode` and/or `--landing-base`, validates, persists, and reloads the project's landing policy before rendering configured/effective output; without either flag, it changes no project data | The command-level `Mutates` declaration keeps the gate in the shared CLI/MCP dispatcher, whose declared flagless-inspection escape permits read-only human/JSON output; either landing flag requires `rw` before the handler runs. Conflicting duplicates and Git-invalid bases are rejected before write | Re-run `project show <slug>` with the prior landing values; no automatic policy-history rollback exists |
