...
- For a spawned child, the runtime's own sandbox enforces the grant, so `dacli`'s cooperative permission model becomes genuinely enforced for exactly these agents. **A role's `grant` and its `runtime` must agree**: a `grant: ro` role is only honest on a runtime that can actually enforce read-only, so `dacli spawn ... --grant ro` on a runtime whose `sandbox_ro` arg list is empty is **refused with exit 3** (`sandboxFor`, `internal/features/execution/execution.go`) — "cannot enforce read-only; spawning an unrestricted process labeled ro would be a lie" — never silently downgraded to rw. Passing `--cooperative` accepts convention-only permissions and spawns with a loud warning instead; an `rw` grant needs no sandbox and always spawns. **How to check before you spawn:** `dacli runtime doctor` (or `dacli runtime list`) prints each runtime's read-only status — a runtime shown as `✗ no read-only mode (ro spawns will be refused)` cannot back a `ro` role, so either give that role an `rw` grant or point its `runtime:` at one that sandboxes.
- **The `rw` direction is the one nothing checks, and it fails silently.** `sandboxFor` returns early for any grant that is not `ro`, so an `rw` role spawns on *any* runtime — including one whose adapter grants no write tool at all. `junior` shipped exactly this way: `grant: rw`, `runtime: cc`, and `cc`'s allowlist is `Read,Grep,Glob,LS` plus the dacli binary. Nothing refuses the spawn; the agent starts, reads its brief, tries to edit a file, discovers it cannot, and the run is spent. Before routing implementation work to a role, read its `runtime:` and then that runtime's `sandbox_ro_args` / `invoke_args` in `.dacli/runtimes/<name>.md` and confirm `Edit` and `Write` are actually in the list. A `grant` says what the workspace *permits*; the runtime's allowlist says what the process can *do*, and only the second one is real (dacli 250).
