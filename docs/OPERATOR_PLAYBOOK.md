...
5. Push and open the task PR with `dacli github push <project> <ref>` and `dacli pr --task <ref> --with-verdicts --auto`. Confirm checks and merged state (`dacli pr status --task <ref>`); then use the configured landing policy, sync/close the task and issue, and record `dacli retro <ref> --well "..." --bad "..." --improve "..."`.
