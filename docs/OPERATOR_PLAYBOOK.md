...
At the pre-cycle boundary, a root-owned loop automatically appends audited
dismissals for redundant `PR opened:` comments only when the generation-zero
canonical PR is uniquely observed merged and the task is terminal with complete
acceptance. Its content-addressed snapshot records every event, task, PR, head,
merge commit, source, and observation time. Reopened generations,
stale/conflicting mappings, unrelated events, and unavailable GitHub state
remain fail-closed.
