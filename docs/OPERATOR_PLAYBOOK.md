...
1. Link and inspect the repository, list existing open and active tasks, then preview inbound work: `dacli github doctor`; `dacli github pull <project> --dry-run`. Compare the proposed issue titles with existing work before the real pull because issue adoption prevents duplicate issue mappings but does not perform semantic deduplication.
