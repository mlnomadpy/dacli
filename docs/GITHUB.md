...
7. **Close both views.** For an explicitly selected nonterminal task, `dacli ship --tasks <ref> --pr --verify "<cmd>"` merges only after checks pass, fetches and inspects the configured base, runs post-landing verification, accepts locally, and then performs scoped GitHub projection so the issue cannot close early. A PR opened for an already-done task retains `Fixes #...`. If a task was integrated without PR-first closure, the next scoped GitHub push closes the issue best-effort from local done state.
