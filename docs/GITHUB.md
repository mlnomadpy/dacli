...
- **Outbound push (`dacli github push <project>`)** projects local state onto Issues, all behind one disclosure gate and all marker-idempotent (§ 4). `--closure-only` is the least-disclosure completion mode: it requires explicit mapped done-task refs and closes only those issues, without publishing bodies, findings, decisions, labels, or milestones.
...
- **PR enrichment + verify verdicts (`dacli pr`).** The PR body is assembled from the task's acceptance criteria and finding notes. A task already accepted as done also carries `Fixes #<issue>`; a nonterminal task deliberately does not, because its issue must stay open until the merged head is observed on fresh trunk and post-landing verification accepts the local task. `dacli pr --with-verdicts` additionally leads the body with a loud trust-grade summary + per-finding verdict tally, and posts the same section plus the verify panel's recorded per-seat verdicts as a single PR **review comment**. `dacli pr --draft` opens the PR as a GitHub draft (§ 9.7).
...
4. **Open the PR.** Push the task branch with `dacli push <ref>`, preview its target with `dacli pr --task <ref> --dry-run`, then run `dacli pr --task <ref> --with-verdicts`. The target resolves as explicit `--base`, configured project landing base, then the linked repository default branch; dacli reports the selected base and fails closed if it cannot resolve one. The enriched PR contains acceptance criteria, findings, and the verify result. It carries `Fixes #<mapped-issue>` only when the local task is already done; a nonterminal task's issue stays open for post-landing acceptance. `dacli integrate --pr --no-merge` is the wave-oriented equivalent when review must remain manual.
