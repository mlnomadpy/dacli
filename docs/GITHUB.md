...
4. **Open the PR.** Push the task branch with `dacli push <ref>`, preview its target with `dacli pr --task <ref> --dry-run`, then run `dacli pr --task <ref> --with-verdicts`. The target resolves as explicit `--base`, configured project landing base, then the linked repository default branch; dacli reports the selected base and fails closed if it cannot resolve one. The enriched PR contains acceptance criteria, findings, the verify result, and `Fixes #<mapped-issue>`. `dacli integrate --pr --no-merge` is the wave-oriented equivalent when review must remain manual.
