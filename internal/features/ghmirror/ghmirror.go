...
func cmdPush(ctx *clikit.Ctx, args []string) error {
	...
	if err := mirrorDecisions(w, repo, decNotes, refTasks, since, idx, ctx.Stdout); err != nil {
		return err
	}
	...
	if findingsAsIssues {
		if err := mirrorFindingIssues(w, repo, findIssueNotes, refTasks, since, idx, ctx.Stdout); err != nil {
			return err
		}
	}
	return nil
...
func mirrorFindingsOnly(w *workspace.Workspace, p *store.Project, repo string, f *clikit.Flags, idx *markerIndex, out io.Writer) error {
	...
	if err := mirrorFindingIssues(w, repo, notes, refTasks, since, idx, out); err != nil {
		return err
	}
	return nil
}
...
// never a duplicate.
func mirrorDecisions(w *workspace.Workspace, repo string, notes []noteFile, refTasks []*store.Task, since time.Time, idx *markerIndex, out io.Writer) error {
	if len(notes) == 0 {
		return nil
	}
...
func mirrorFindingIssues(w *workspace.Workspace, repo string, notes []noteFile, refTasks []*store.Task, since time.Time, idx *markerIndex, out io.Writer) error {
	if len(notes) == 0 {
		return nil
	}
