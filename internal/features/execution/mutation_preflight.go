...
func handoffChannelPreamble(runDir string, planned []string) string {
	...
	return fmt.Sprintf("\nROOT HANDOFF CONTRACT (planned capabilities: %s): If commit, event, or GitHub publication is unavailable after useful edits or verification, do not claim success and do not switch harness or seek broader authority. Write structured JSON to %s with schema %q and fields commit_message, verification:[{command,exit_code,result}], unresolved_findings:[], failed_operation, failure_class, stderr, safe_owner_next_action. For a planned git-metadata-write handoff, the parent will independently re-observe task/run/branch, claims, paths, contents, tree, author/trailers, and verification evidence, then create the exact commit without granting you shared-Git write access. Continue preserving the worktree for root.\n", label, path, store.RootHandoffSchema)
}
