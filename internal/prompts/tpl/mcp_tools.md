...
## cleanup_repository
Plan repository cleanup before mutation, apply only the exact safe operations from a reviewed plan id, or restore one quarantined generated artifact by its audited identity. Always call with dry_run first. A changed branch, worktree, task, remote ref, PR state, or generated-artifact digest changes the plan id and makes apply_safe refuse rather than guessing. Eligible generated artifacts are quarantined individually; restore verifies the recorded digest and refuses to overwrite an existing source. Preserved items include an actionable reason; applied items retain exact recovery data in an audit record.
...
- Agent lifecycle: spawn (--detach backgrounds it and returns a run-id immediately; --claim "<path,path>" declares its edit paths so parallel children stay disjoint; --advise PREVIEWS the calibrated role×model×runtime band, taint status, and requested token-ceiling capability without launching; --max-tokens N passes a declared runtime ceiling flag or refuses, while --allow-advisory-tokens explicitly permits an unenforced launch), wait (block until detached runs finish, then finalize outcome), agents --tail (each child's last transcript line — thinking vs. hung), logs -f (follow a run's transcript), kill (whole process tree, SIGTERM→SIGKILL).
